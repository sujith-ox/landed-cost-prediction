...
def validate_model_input(df: pd.DataFrame) -> None:
    ...
    if "purchase_amount_per_uom_in_vendor_currency" in df.columns:
        vals = pd.to_numeric(
            df["purchase_amount_per_uom_in_vendor_currency"], errors="coerce"
        )
        if (vals.dropna() <= 0).any():
            raise ValueError(
                "purchase_amount_per_uom_in_vendor_currency must be positive."
            )
...
def score_landed_cost(df: pd.DataFrame, artifact: Dict[str, Any]) -> pd.DataFrame:
    ...
    if target_col in result.columns:
        ...
        result["residual_pct"] = result["absolute_residual"] / result[
            "predicted_landed_cost"
        ].replace(0, np.nan)
        result["is_cost_anomaly"] = (result["absolute_residual"] >= threshold).astype(
            int
        )
    else:
        for col in [
