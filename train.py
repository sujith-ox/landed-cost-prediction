...
from pipeline import build_artifact, create_pipeline, regression_metrics, score_landed_cost
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--scored-path",        default=os.path.join("models", "training_scores.csv"))
...
def main() -> None:
    ...
    print(f"  R²   {cv_r2.mean():.4f} ± {cv_r2_std:.4f}   folds: {np.round(cv_r2, 3).tolist()}")
    ...
    test_groups   = set(rng.choice(unique_groups, size=max(1, int(len(unique_groups) * args.test_size)), replace=False))
    ...
    print(f"  Held-out test — R²: {metrics['r2']:.4f}  MAE: ₹{metrics['mae']:,.0f}  MAPE: {metrics['mape']:.2%}")
    ...
    artifact   = build_artifact(
        ...
        metrics=metrics,
        cv_metrics={"r2": float(cv_r2.mean()), "r2_std": float(cv_r2_std),
                    "mae": cv_mae, "mape": cv_mape, "n_splits": n_splits},
    ...
    scored.sort_values("absolute_residual", ascending=False).to_csv(args.scored_path, index=False)
