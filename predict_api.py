...
from features import MODEL_FEATURES, build_model_frame
from pipeline import score_landed_cost
...
HIGH_CONF_ROUTES   = {      # ports with ≥30 historical slabs — higher confidence
    "jebel ali", "mersin", "gemlik", "izmir", "aliaga", "venezia",
}
...
class SlabRequest(BaseModel):
    ...
    # Slab identity
    product_name: Optional[str] = Field(None, description="Stone type (e.g. RAINFOREST GREEN)")
    ...
    port_of_shipment:    Optional[str] = Field(None, description="Origin port (e.g. MERSIN)")
...
def score_from_db(request: ScoreDbRequest) -> Dict[str, Any]:
    ...
    output_cols = [c for c in [
        "id", "mrn_id", "product_name", "color", "country",
        "vendor_name", "broker_name", "port_of_shipment",
        "weight", "purchase_amount_per_uom_in_vendor_currency",
        "actual_landed_cost", "predicted_landed_cost",
        "residual", "residual_pct", "is_cost_anomaly",
